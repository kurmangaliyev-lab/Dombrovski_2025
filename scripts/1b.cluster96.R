...
# 1. Load and Prep
data0 <- CreateSeuratObject(Read10X("../data1/Mark5/outs/filtered_feature_bc_matrix/"), project = "APF_96h")
data0$Perc_MT <- PercentageFeatureSet(data0, "mt:")

data0 <- AddMetaData(
  data0,
  read.table("../data1/multiplex1/Mark5/demuxlet0.best", header = T, sep = "\t") %>% 
...
saveRDS(data0, "data/data1_96h.rds")
...
data0 <- RenameIdents(data0, "0"="LPLC2", "2"="LPLC1", "1"="LC4", "3"="X")
...
ggsave(filename = "results/cluster_96h.pdf", units = "in", width = 7, height = 3.5)

saveRDS(data0, file = "data/data2_96h.rds")
