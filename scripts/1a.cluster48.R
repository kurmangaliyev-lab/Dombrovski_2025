...
# 1. Load and Prep
data0 <- CreateSeuratObject(Read10X("../data1/Mark3/outs/filtered_feature_bc_matrix/"), project = "APF_48h")
data0$Perc_MT <- PercentageFeatureSet(data0, "mt:")

data0 <- AddMetaData(
  data0,
  read.table("../data1/multiplex1/Mark3/demuxlet0.best", header = T, sep = "\t") %>% 
    select(cellID = BARCODE, is_sng = DROPLET.TYPE, genotype = SNG.BEST.GUESS) %>% 
    mutate(genotype = ifelse(is_sng == "SNG", genotype, is_sng)) %>%
    mutate(group = "X") %>%
    mutate(group = ifelse(genotype %in% c("line_399","line_535","line_555"), "early", group)) %>%
    mutate(group = ifelse(genotype %in% c("line_730","line_799","line_805"), "late", group)) %>%
    column_to_rownames("cellID")
)

saveRDS(data0, "data/data1_48h.rds")
...
ggsave(filename = "results/cluster_48h.pdf", units = "in", width = 7, height = 3.5)

saveRDS(data0, file = "data/data2_48h.rds")
